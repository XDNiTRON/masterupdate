...
class GoogleDriveHelper:
    ...
    def __upload_empty_file(self, path, file_name, mime_type, parent_id=None):
        ...
        file_metadata = {
            'name': file_name,
            'description': 'Uploaded using ANonYmoUSFriEND',
            'mimeType': mime_type,
        }
            ...
            if meta.get("mimeType") == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                if INDEX_URL is not None:
                    ...
                    if SHORTENER is not None and SHORTENER_API is not None:
                        ...
                        buttons.buildbutton("🏓 Index Link 🏓", siurl)
                        if VIEW_LINK:
                            buttons.buildbutton("🎥 Stream Link 🎥", siurls)
                    else:
                        buttons.buildbutton("🏓 Index Link 🏓", url)
                        if VIEW_LINK:
                            buttons.buildbutton("🎥 Stream Link 🎥", urls)
                if BUTTON_FOUR_NAME is not None and BUTTON_FOUR_URL is not None:
                    buttons.buildbutton(f"{BUTTON_FOUR_NAME}", f"{BUTTON_FOUR_URL}")
                if BUTTON_FIVE_NAME is not None and BUTTON_FIVE_URL is not None:
                    buttons.buildbutton(f"{BUTTON_FIVE_NAME}", f"{BUTTON_FIVE_URL}")
    ...
    def drive_list(self, fileName):
        ...
        if response["files"]:
                ...
                if file.get('mimeType') == "application/vnd.google-apps.folder":  # Detect Whether Current Entity is a Folder or File.
                    ...
                    if INDEX_URL is not None:
                        ...
                        if SHORTENER is not None and SHORTENER_API is not None:
                            ...
                            msg += f' <b>| <a href="{siurl}">🏓 Index Link</a></b>'
                            if VIEW_LINK:
                                msg += f' <b>| <a href="{siurls}">🎥 Stream Link</a></b>'
                        else:
                            msg += f' <b>| <a href="{url}">🏓 Index Link</a></b>'
                            if VIEW_LINK:
                                msg += f' <b>| <a href="{urls}">🎥 Stream Link</a></b>'
                msg += '<br><br>'
                content_count += 1
            ...
            buttons = button_build.ButtonMaker()   
            buttons.buildbutton("🎁 VIEW SEARCH LIST 🎁", f"https://telegra.ph/{self.path[0]}")
